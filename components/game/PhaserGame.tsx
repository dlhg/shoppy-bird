...
class GameScene extends Phaser.Scene {
    ...
    private pipesNeededForNextBonus: number = BONUS_CONSTANTS.INITIAL_PIPES_FOR_BONUS;
    ...
    preload(): void {
        ...
        this.load.image(birdAssetKey, 'shoppy.png');
        console.log(`Phaser Preload: Attempting to load 'shoppy.png' as '${birdAssetKey}'`);

        this.load.once(Phaser.Loader.Events.FILE_LOAD_ERROR, (fileObj: Phaser.Loader.File) => {
            if (fileObj.key === birdAssetKey) {
                console.error(`Phaser Loader Error: Failed to load ${birdAssetKey} (shoppy.png)! Check path/integrity. URL: ${fileObj.url}`, fileObj);
            }
        });
        this.load.on(Phaser.Loader.Events.FILE_COMPLETE, (key: string, type: string, _fileData: Phaser.Loader.File) => {
            if (key === birdAssetKey) {
                console.log(`Phaser Loader: ${birdAssetKey} (shoppy.png) load attempt complete.`);
                if (this.textures.exists(birdAssetKey)) {
                    ...
                    console.log(`Phaser Loader: Texture ${birdAssetKey} exists in manager. Key: ${texture.key}, Width: ${texture.width}, Height: ${texture.height}, Source: ${texture.source[0]?.source?.src}`);
                     if(texture.key === '__MISSING' || texture.key === '__DEFAULT' || texture.width <= 1 || texture.height <= 1){
                        console.warn(`Phaser Loader: ${birdAssetKey} appears to be a missing/default or invalid small texture after loading.`);
                     }
                } else {
                     console.warn(`Phaser Loader: ${birdAssetKey} NOT FOUND in texture manager after load completion event.`);
                }
            }
        });
    ...
    create(): void {
        ...
        this.pipesNeededForNextBonus = BONUS_CONSTANTS.INITIAL_PIPES_FOR_BONUS; 
        ...
        const birdAssetKey = 'shoppyBirdAssetActual';
        this.bird = this.physics.add.sprite(BIRD_CONSTANTS.BIRD_START_X, BIRD_CONSTANTS.BIRD_START_Y, birdAssetKey) as GameScene['bird'];
        console.log(`Phaser Create: Bird sprite created with texture key '${birdAssetKey}'.`);
        
        let useFallback = false;
        if (this.textures.exists(birdAssetKey)) {
            const birdTexture = this.textures.get(birdAssetKey);
            console.log(`Diagnostic Create: Texture '${birdAssetKey}' found in manager. Key: ${birdTexture.key}, Width: ${birdTexture.width}, Height: ${birdTexture.height}`);
            if (birdTexture.key === '__MISSING' || birdTexture.key === '__DEFAULT' || birdTexture.width <= 1 || birdTexture.height <= 1) {
                console.warn(`Diagnostic Create: '${birdAssetKey}' is a placeholder/default or invalid small texture. Applying BLUE fallback.`);
                useFallback = true;
            } else {
                 console.log(`Diagnostic Create: '${birdAssetKey}' seems valid and will be used for the bird.`);
            }
        } else {
            console.error(`Diagnostic Create: Texture '${birdAssetKey}' NOT FOUND in texture manager. This means 'shoppy.png' failed to load. Applying BLUE fallback.`);
            useFallback = true;
        }

        if (useFallback) {
            const fallbackKey = 'fallbackBirdBlueTexture';
            if (!this.textures.exists(fallbackKey)) {
                const fallbackGraphics = this.make.graphics({ fillStyle: { color: 0x0000ff } }, false); // Blue
                fallbackGraphics.fillRect(0, 0, 34, 24);
                fallbackGraphics.generateTexture(fallbackKey, 34, 24);
                fallbackGraphics.destroy();
                console.log(`Diagnostic Create: Generated '${fallbackKey}'.`);
            }
            this.bird.setTexture(fallbackKey);
        }

        this.bird.displayWidth = 34; 
        this.bird.displayHeight = 24; 
        this.bird.body.setSize(34, 24); // Ensure physics body matches display size
    ...
    spawnSingleBonusItem(): void {
        ...
        const x = GAME_CONSTANTS.GAME_WIDTH + Phaser.Math.Between(30, 100); 
        const y = Phaser.Math.Between(GAME_CONSTANTS.GAME_HEIGHT * 0.1, GAME_CONSTANTS.GAME_HEIGHT * 0.9); 
